        ...
        Profile.findOne({ user: req.user.id })
          .then(profile => {
            if (profile) {
              ...
              profileInput.username = user.username;
              profileInput.avatar = user.avatar;
...
//@route   /api/profiles/view/:username
//@desc    search for profiles
//@access  Private
router.get(
  "/view/:username",
  ...
  (req, res) => {
    ...
    Profile.findOne({ username: username }).then(profile => {
      if (profile) {
        ...
        errors.view = "Profile doesn't exist";
        res.status(400).json(errors);
      }
    });
  }
);
//@route   /api/profiles/view/following
//@desc    returns the viewed profiles following
//@access  Private
router.post(
  "/view/following",
  passport.authenticate("jwt", { session: false }),
...
//@route   /api/profiles/view/followers
//@desc    returns the viewed profiles followers
//@access  Private
router.post(
  "/view/followers",
  passport.authenticate("jwt", { session: false }),
